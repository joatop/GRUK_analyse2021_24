...
```{r}
ggplot(res.natopen.GRUK[res.natopen.GRUK$fp_ind=="Light2",], aes(x=Totaldekning_fremmedearter, y=scaled_value)) +
  geom_point() +
  xlab("Alien species cover (%)") + ylab("Scaled Light2 value (GRUK data)")

ggplot(res.natopen.GRUK[res.natopen.GRUK$fp_ind=="Light2",], aes(x=Dekning_problemarter, y=scaled_value)) +
  geom_point() +
  xlab("Problem species cover (%)") + ylab("Scaled Light2 value (GRUK data)")

ggplot(res.natopen.GRUK[res.natopen.GRUK$fp_ind=="Light2",], aes(x=Dekning_busker_busksjikt, y=scaled_value)) +
  geom_point() +
  xlab("Shrub species cover (%)") + ylab("Scaled Light2 value (GRUK data)")

ggplot(res.natopen.GRUK[res.natopen.GRUK$fp_ind=="Light2",], aes(x=Dekning_tresjikt, y=scaled_value)) +
  geom_point() +
  xlab("Tree species cover (%)") + ylab("Scaled Light2 value (GRUK data)")
```
...
## Import Norway map
nor <- st_read("data/mapdata/Norway outline/outlineOfNorway_EPSG25833.shp") #%>%
#  st_as_sf() %>%
#  st_transform(crs = st_crs(myr))

## Import region map
reg <- st_read("data/mapdata/Norway outline/regions.shp") #%>%
#  st_as_sf() %>%
...
tm_shape(regnor) +
  ...
  tm_dots('Light2',midpoint=NA, palette=tmaptools::get_brewer_pal("YlOrRd", 7, plot = FALSE), scale=2, legend.show = FALSE) + # 
...
tm_shape(regnor, bbox=boks) +
  ...
  tm_graticules() +
  tm_shape(res.natopen.GRUK2) +
  tm_dots('Light2',midpoint=NA, palette=tmaptools::get_brewer_pal("YlOrRd", 7, plot = FALSE), scale=3, legend.show = FALSE) + # 
  tm_layout(main.title = "Light index (upper), natopen GRUK",legend.position = c("right", "bottom"), main.title.size=1.2) + 
  tm_add_legend(type = "fill", 
                col = c(tmaptools::get_brewer_pal("YlOrRd", 7, plot = FALSE),'grey'),
                labels = c("0.3 - 0.4", "0.4 - 0.5", "0.5 - 0.6", "0.6 - 0.7", 
