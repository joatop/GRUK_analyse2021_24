---
title: "GRUK_veg_analyse_2021_2025"
...
#excel_sheets("P:/15211700_gruk_2021_2024/Data/Filer som endres i R/GRUK_alle_artsdata_2020-2025.xlsx")

GRUK.species <- read_excel("P:/15211700_gruk_2021_2024/Data/Filer som endres i R/GRUK_alle_artsdata_2020-2025.xlsx",sheet="Arter i ruter")
GRUK.ruter <- read_excel("P:/15211700_gruk_2021_2024/Data/Filer som endres i R/GRUK_alle_artsdata_2020-2025.xlsx",sheet="Ruter")
GRUK.sirkler <- read_excel("P:/15211700_gruk_2021_2024/Data/Filer som endres i R/GRUK_alle_artsdata_2020-2025.xlsx",sheet="Sirkler")
GRUK.polygoner <- read_excel("P:/15211700_gruk_2021_2024/Data/Filer som endres i R/GRUK_alle_artsdata_2020-2025.xlsx",sheet="Polygoner")
...
names(GRUK.species)<-c("ObjectID","GlobalID","ParentGlobalID","Year","PolygonID","RuteID","Norsknavn","Latinsknavn","art_dekning","RAkat","CreationDate","Creator","EditDate","Editor","Navn","karplantenavn","karplantedekning","veg_html_row","x")
...
GRUK.species$art_dekning <- as.numeric(GRUK.species$art_dekning)

# merge species data with indicators
GRUK.species.ind <- merge(x=GRUK.species[,c("Species", "art_dekning", "Year", "ParentGlobalID","PolygonID","RuteID")], 
...
names(GRUK.ruter)
names(GRUK.sirkler)
GRUK.variables <- merge(x=GRUK.ruter[,c(2,5:12,20:24,26,53:54)], 
...
# fixing data type issues
summary(GRUK.variables)
GRUK.variables$Totaldekning_fremmedearter <- as.numeric(GRUK.variables$Totaldekning_fremmedearter)
GRUK.variables$Dekning_busker_busksjikt  <- as.numeric(GRUK.variables$Dekning_busker_busksjikt)
GRUK.variables$erosjon_prosent    <- as.numeric(GRUK.variables$erosjon_prosent)
GRUK.variables$Dekning_karplanter_feltsjikt     <- as.numeric(GRUK.variables$Dekning_karplanter_feltsjikt)
GRUK.variables$Dekning_moser          <- as.numeric(GRUK.variables$Dekning_moser)
GRUK.variables$Dekning_lav           <- as.numeric(GRUK.variables$Dekning_lav)
GRUK.variables$Dekning_strø           <- as.numeric(GRUK.variables$Dekning_strø)
GRUK.variables$Dekning_bar_substrat    <- as.numeric(GRUK.variables$Dekning_bar_substrat)
GRUK.variables$Dekning_nakentberg    <- as.numeric(GRUK.variables$Dekning_nakentberg)
GRUK.variables$Totaldekning_vedplanter_feltsjikt    <- as.numeric(GRUK.variables$Totaldekning_vedplanter_feltsjikt)
GRUK.variables$Dekning_tresjikt    <- as.numeric(GRUK.variables$Dekning_tresjikt)
GRUK.variables$Dekning_problemarter    <- as.numeric(GRUK.variables$Dekning_problemarter)
...
#write.csv(natopen.ref.cov.val,"C:/Users/joachim.topper/OneDrive - NINA/work/R projects/github/GRUK_analyse2021_24/output/ref.csv", dec=",")
...
#saveRDS(results.natopen.GRUK, "data/cache/results.natopen.GRUK2025.RDS")
```
...
# Use cache data
results.natopen.GRUK<-readRDS(paste0(here::here(),"/data/cache/results.natopen.GRUK2025.RDS"))
```
...
Aggregation of indicators per circle
```{r, message = FALSE, warning = FALSE}

## continuing with 2-sided
res.natopen.GRUK_wide <- results.natopen.GRUK[['2-sided']]

## aggregating the functional plant indicators by the 'verste-styrer' principle
# and aggregating fpi and the other indicators by arithmetic mean
...
## aggregating fpi and other indicators by beta-regression null-models
expit <- function(L) exp(L) / (1+exp(L)) # expit function for backtransforming from logit-link
res.natopen.GRUK_wide$ec.beta_agg <- NA
for (i in 1:nrow(res.natopen.GRUK_wide) ) {
  
  dat <- as.vector(unlist(res.natopen.GRUK_wide[i,c("fpi","aliens","erosion","shrub")]))
  ...
  res.natopen.GRUK_wide[i,"ec.beta_agg"] <- expit(summary(betareg(dat~1))$coefficients$mean[1])

}

## aggregating all indicators in a circle by the 'verste-styrer' principle
res.natopen.GRUK_wide <- res.natopen.GRUK_wide %>%
  rowwise() %>%
  mutate(ec.vs = min(Light1,Light2,Nitrogen1,Nitrogen2,Soil_disturbance1,Soil_disturbance2,aliens,erosion,shrub,na.rm=T)) %>%
  mutate(ec.vs = na_if(ec.vs,Inf))


```

aggregating circle indices (the verste-styrer aggregates) into polygon indices
```{r, include = F}
...
#saveRDS(ec_polygon, "data/cache/ec.polygon.GRUK2025.RDS")
```

```{r, include = F}

# Use cache data
ec_polygon<-readRDS(paste0(here::here(),"/data/cache/ec.polygon.GRUK2025.RDS"))
```

Plotting the verste-styrer aggregate for each fjord-region (indre, midtre og ytre)
```{r}
...
Calculating an aggregated index for GRUK as a whole and for the three fjord areas
(based on verste-styrer indices at circle level)
```{r}
...
summary(ec.omr.vs)
expit(
  c(summary(ec.omr.vs)$coefficients$cond[1,1],
    summary(ec.omr.vs)$coefficients$cond[1,1]+summary(ec.omr.vs)$coefficients$cond[2,1],
    summary(ec.omr.vs)$coefficients$cond[1,1]+summary(ec.omr.vs)$coefficients$cond[3,1])
)


```

For further visualizations of the single indicator results we need to re-arrange the results-objects from wide to long format (note that there is both a lower and an upper condition indicator for each of the functional plant indicators).
...
We can show the resulting scaled values for the functional plant indicators as Violin plots for each plant indicator in both T2-C7 and T2-C8 or for GRUK alltogether
```{r, message=FALSE, warning=FALSE}
...
ggplot(aes(x=factor(Kartleggingsenhet_1til5000), y=scaled_value, fill=ec.index)) + 
  geom_hline(yintercept=0.6, linetype="dashed") + 
  geom_violin(color = NA) +
  #  geom_boxplot(width=0.2, color="grey") +
  geom_point(size=1, shape=16, color="black") +
  facet_wrap(~factor(ec.index,levels=c("CC1","SS1","RR1","Light1","Nitrogen1","Soil_disturbance1",
                                     "CC2","SS2","RR2","Light2","Nitrogen2","Soil_disturbance2"),
...
res.natopen.GRUK %>%
  ...
  #  geom_boxplot(width=0.2, color="grey") +
  geom_point(size=1, shape=16, color="black", position = position_jitter(seed = 1, width = 0.05)) +
  facet_wrap(~dir, nrow = 2, , scale="free", labeller = labeller(dir = c("upper"="Upper indicator", "lower"="Lower indicator"))) + 
  xlab("") + 
  ylab("Scaled indicator value") + 
  theme(legend.position="none") +
  theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0.2))
...
Aggregated condition index against polygon assessment
```{r}
res.natopen.GRUK_wide %>%
...
We can test if the functional plant indicator indices correlate with the circle based indicator data
```{r, eval=F}
...
for (i in x) {
  for (j in y1) {
    df <- res.natopen.GRUK[res.natopen.GRUK$ec.index==j,c(i,'scaled_value')]
    names(df) <- c("xx","yy")
...
From this overview we focus on negative trends (i.e. condition variables increase towards worse and condition indicators decrease towards worse) and build more appropriate models taking the data structure into account
```{r}
res.natopen.GRUK <- res.natopen.GRUK %>%
...
summary( glmmTMB(scaled_value2 ~ Dekning_tresjikt +(1|LokalitetID), family=beta_family(), data=res.natopen.GRUK[res.natopen.GRUK$ec.index=="Light1",]) )

summary( glmmTMB(scaled_value2 ~ erosjon_prosent +(1|LokalitetID), family=beta_family(), data=res.natopen.GRUK[res.natopen.GRUK$ec.index=="Light2",]) )

summary( glmmTMB(scaled_value2 ~ Totaldekning_fremmedearter +(1|LokalitetID), family=beta_family(), data=res.natopen.GRUK[res.natopen.GRUK$ec.index=="Nitrogen2",]) )

summary( glmmTMB(scaled_value2 ~ Dekning_tresjikt +(1|LokalitetID), family=beta_family(), data=res.natopen.GRUK[res.natopen.GRUK$ec.index=="Soil_disturbance1",]) )

summary( glmmTMB(scaled_value2 ~ Dekning_problemarter +(1|LokalitetID), family=beta_family(), data=res.natopen.GRUK[res.natopen.GRUK$ec.index=="Soil_disturbance2",]) )

summary( glmmTMB(scaled_value2 ~ Totaldekning_fremmedearter +(1|LokalitetID), family=beta_family(), data=res.natopen.GRUK[res.natopen.GRUK$ec.index=="CC2",]) )

summary( glmmTMB(scaled_value2 ~ Dekning_problemarter +(1|LokalitetID), family=beta_family(), data=res.natopen.GRUK[res.natopen.GRUK$ec.index=="CC2",]) )
...
res.natopen.GRUK2$Lokalitetskvalitet <- factor(res.natopen.GRUK2$Lokalitetskvalitet,levels=c("Lav kvalitet","Moderat kvalitet","Høy kvalitet","Svært høy kvalitet"))
res.natopen.GRUK2$Tilstand <- factor(res.natopen.GRUK2$Tilstand, levels=c("Dårlig","Moderat","God"))
...
## polygon condition evaluation, maps
```{r}
...
```
## polygon biodiversity evaluation, maps
```{r}
...
```
## polygon site quality evaluation, maps
```{r}
tm_shape(regnor, bbox=boks) +
  tm_fill('GID_0', labels="", title="", legend.show = FALSE) + 
  tm_borders() +
